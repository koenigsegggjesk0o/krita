#!/usr/bin/env python3
"""v0.39 release: fetch ALL real-engine artifacts from the builder repo's
...
Loop-56 highlights this release carries: SILHOUETTE CONTOUR FEATHERING —
buildSurfaceItems detects contour edges of the guide-surface mesh (shared
with a back-facing neighbour, or a mesh boundary) and emits a SceneFeather
quad per visible segment: the edge extruded 1.5px along its outward screen
normal with a per-vertex alpha ramp (kContourFeatherAlpha x texel alpha at
the base, 0 at the rim), drawn as a triangle strip by the painter. The
non-AA drawVertices path now reads antialiased — most visible on close-ups.
Interior edges never feather, so the translucent fill is never
double-blended. 6 new unit tests (suite 173).
...
def main():
    ...
    if status == 200:
        ...
        status, rel = api(f'{HEAD}/repos/{REPO}/releases', 'POST', {
            ...
            'name': 'Feather-Krita v0.39 — silhouette contour feathering',
