#!/usr/bin/env python3
"""v0.40 release: fetch ALL real-engine artifacts from the builder repo's
...
Loop-57 highlights this release carries: the LIGHT PRESET ROW — three
one-tap curated sun setups (Noon az90/el80 @100%, Golden hour az45/el8
@55%, Rim az270/el20 @85%) feeding the rig through its canonical setters
(the same ones the orbit drag and power dial use), as glass chips above
the dial. The HUD, scene lighting and .feather persistence all see the
same state; taps are gesture-isolated from the canvas. 5 new tests
(suite 178).
...
def main():
    ...
    if status == 200:
        ...
        status, rel = api(f'{HEAD}/repos/{REPO}/releases', 'POST', {
            ...
            'name': 'Feather-Krita v0.40 — light preset row',
