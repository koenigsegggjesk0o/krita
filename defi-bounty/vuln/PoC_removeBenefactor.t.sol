...
// ============================================================================
// PoC_removeBenefactor.t.sol
// ----------------------------------------------------------------------------
// Verifies the CRITICAL vulnerability in PSM.removeBenefactor():
//   `delete benefactorState[benefactor].config` does NOT clear nested mappings
//   (delegatedSigners, approvedBeneficiaries, fee exemptions, custom fees).
//   After remove + re-add, an attacker retains delegated-signer + approved-
//   beneficiary status WITHOUT re-confirmation, and can drain funds via swap().
//
// Result: 4/4 tests PASS — vulnerability CONFIRMED.
//
// Test layout (relative to a Foundry project root):
//   src/PSM.sol                  (copy of contracts/PSM.sol with imports adjusted)
//   src/IPSM.sol                 (from contracts/deps/IPSM.sol)
//   src/CollateralStateMap.sol   (from contracts/deps/CollateralStateMap.sol)
//   src/oracle/IOracleFeed.sol   (from contracts/deps/IOracleFeed.sol)
//   src/access/SingleAdminAccessControl.sol     (from contracts/deps/...)
//   src/access/ISingleAdminAccessControl.sol    (from contracts/deps/...)
//   test/PoC_removeBenefactor.t.sol   <-- THIS FILE
//
// foundry.toml remappings:
//   @openzeppelin/contracts/=lib/openzeppelin-contracts/contracts/
// (openzeppelin v5.0.2; via_ir = true; solc 0.8.30)
//
// Run:   forge test -vvvv --match-contract PoC_removeBenefactor
// ============================================================================
...
/// @notice Demonstrates that PSM.removeBenefactor() fails to clear nested
///         mappings in BenefactorConfig, allowing a previously-authorized
///         delegated signer + beneficiary to regain full swap authority after
///         a remove+re-add cycle - with NO re-confirmation.
contract PoC_removeBenefactor is Test {
    ...
    function test_RemoveBenefactor_AttackerCanSwapAfterReAdd() public {
        ...
        emit log_named_decimal_uint(
            "benefactorA collateral loss", benefactorCollateralBefore - benefactorCollateralAfter, 18
        );
    }
