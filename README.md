...
| Tests | `test/` | 39 tests: native dab contract, preset loading, texture compositing, mirror, raycast, PNG export round-trip, project save/load round-trip, GIF replay frames, glTF structure, preset-library seeding, camera damping, plus 9 GUI wiring tests (incl. ticker-muting). |
...
```bash
flutter pub get
flutter test          # 39-test regression suite (loads the real native bridge)
