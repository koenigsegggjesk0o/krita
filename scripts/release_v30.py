#!/usr/bin/env python3
"""v0.30 release: fetch ALL real-engine artifacts from the builder repo's
green build-app run (linux zip + windows zip + android APK, each bundling
the unmodified Krita v6.0.4 engine behind the preset-families ABI — the
engine now enumerates brush presets and reports each preset's declared
paintop family through its own container parser) and publish on
koenigsegggjesk0o/krita.
...
BODY = """## Highlights — the engine can now enumerate brush presets with their paintop families

### Preset-families ABI campaign (loop-47)
- New thin C ABI: `krita_brush_preset_scan(handle, dir)` walks a preset
  folder RECURSIVELY and parses every .kpp through the REAL engine's own
  container path (ZIP KoStore archives, legacy PNG zTXt stock presets,
  bare XML shapes — the exact extraction `krita_brush_load_preset` uses,
  factored into a shared helper so scan and load can never drift).
  `krita_brush_preset_count/_name` serve the scan result; new
  `krita_brush_preset_family` + `krita_brush_preset_path` expose the
  declared paintop family ("paintbrush", "eraser", "spray", ...) and a
  loadable absolute path per entry.
- Proven in CI at BOTH layers against the two real Krita stock fixtures
  (engine run 35539069506 + app run 35540521398, all jobs green): the C++
  smoke scans the fixture dir and asserts both fixtures surface with
  family == paintbrush plus a full load_preset round-trip; the Dart
  smoke re-proves the same gates through the app's own FFI bindings.
- Editor integration: the preset library now upgrades each preset's
  paintopId with the ENGINE-declared family whenever the real engine is
  present (matched by file base name, separator-safe, defensive without
  the engine — the Dart parse remains the fallback authority). The
  bundled preset seeding contract is covered by 4 new unit tests
  (test/preset_families_ux_test.dart).
- Zero Krita source changes — the campaign lives entirely in the thin
  C ABI wrapper, the CI gates and the Dart layer.

## Also in this build
- Everything from v0.29: complete Android DT_NEEDED load closure with a
...
preset enumeration with paintop families + flow and hardness
user-controllable + paintop family identity end-to-end AND visible
across the editor UI + on-device-proven Android load closure.
...
def main():
    global APP_RUN_ID
    out_dir = '/home/z/fkr-step1/build/release_v30'
    ...
    payload = json.dumps({
        ...
        'name': 'Feather-Krita v0.30 — engine-side preset scan with paintop families',
        'body': body,
        'draft': False,
        'prerelease': False,
    }).encode()
