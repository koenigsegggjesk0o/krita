...
| Tests | `test/` | 52 tests: native dab contract, preset loading, texture compositing (+ per-stroke undo coalescing), mirror, raycast, PNG export round-trip, project save/load round-trip (+ texture re-render), GIF replay frames, glTF structure, preset-library seeding, camera damping, MP4/H.264 export (encoder modes, muxer structure, ffmpeg decode gate), plus 9 GUI wiring tests (incl. ticker-muting). |
...
```bash
flutter pub get
flutter test          # 52-test regression suite (loads the real native bridge)
