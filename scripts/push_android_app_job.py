#!/usr/bin/env python3
"""Push the updated build-app.yml (adds build-android-real-engine job) to the
...
def main():
    local = open('/home/z/fkr-step1/build/builder_files/build-app.yml').read()
    job = open('/home/z/fkr-step1/build/android_app_job.yml').read()
    new_content = local.rstrip('\n') + '\n' + job

    try:
        import yaml
        d = yaml.safe_load(new_content)
        assert 'build-android-real-engine' in d['jobs']
        print('YAML parse OK,', len(new_content.splitlines()), 'lines, jobs:', list(d['jobs'].keys()))
    except ImportError:
    ...
    body = json.dumps({
        'message': 'loop-36: add build-android-real-engine job (jniLibs bundling of the real NDK engine, roadmap c app wiring)',
    ...
    for attempt in range(3):
        try:
            disp = http(f'https://api.github.com/repos/{REPO}/actions/workflows/build-app.yml/dispatches',
