...
BODY = """## Highlights — the key light now survives save/load

### Light rig persistence in .feather projects (loop-53)
- Documents now carry an OPTIONAL additive `light` block:
  `{azimuthDeg, elevationDeg, intensity}` — the Light tool's sun sky
  position and diffuse intensity survive save/load round-trips.
- Format version STAYS 2 (the loop-18 camera precedent): older builds
  ignore the unknown key, and older documents load with the rig at its
  defaults, which reproduce the legacy fixed key light exactly.
- The stored quantity is the rig's CANONICAL parametrization (sun sky
  position), not the derived direction vector — the JSON round-trip
  involves no trig, and the rig's setters clamp elevation into
  [-30°, +88°] and intensity into [0, 1] on load, so a hand-edited or
  corrupt file can never degenerate the light at the zenith pole
  (tested: azimuth 400° wraps to 40°, elevation 200° clamps to +88°,
  intensity 7 clamps to 1.0).

### Still true from loop-50..52
- The Light tool is a real scene-light controller: one-finger drag
  orbits the sun around the model, with a projected gizmo and a live
  HUD readout; stroke ribbons re-light per frame through the Lambert
  path.
...
- Full test suite 163/163 green (5 new: light roundtrip, exact restore
  incl. the renderer direction vector, legacy-doc rig-untouched,
  out-of-range clamping, default-document default-rig).
...
def main():
    ...
    os.makedirs('/tmp/fkr_rel36', exist_ok=True)
    files = {}
    for name, (asset, _) in ARTIFACTS.items():
        za = f'/tmp/fkr_rel36/{name}.zip'
        out = f'/tmp/fkr_rel36/{asset}'
        ...
        with zipfile.ZipFile(za) as z:
            z.extractall(f'/tmp/fkr_rel36/x_{name}')
        inner = os.path.join('/tmp/fkr_rel36/x_' + name, asset)
        if not os.path.exists(inner):
            # some zips nest a directory
            for root, _, fs in os.walk(f'/tmp/fkr_rel36/x_{name}'):
                if asset in fs:
    ...
    if status == 200:
        ...
        status, rel = api(f'{HEAD}/repos/{REPO}/releases', 'POST', {
            ...
            'name': 'Feather-Krita v0.36 — light rig persistence',
