...
// and the *actions* testable through the real engine.
//
...
// (B/E/V/L/G/P/M/S/F/[ /]/Delete/Escape/Digits/Plus/Minus/Space/Tab)
// are registered without modifiers; when a text field has focus those
// keys are consumed by the field and never reach this node, so typing
// in a Save-As filename box is unaffected.
...
///
/// Used by the production host (lib/screens/main_screen.dart) to wrap the
/// [EditorScreen] layout shell. The host builds the bindings map from its
/// own engine handlers (undo, redo, tool switch, save, export, camera
/// reset, …) so a single key press drives the real engine — no duplicated
/// state.
