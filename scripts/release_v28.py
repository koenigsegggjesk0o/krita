...
TAG = 'v0.28-brush-picker-families'
...
BODY = """## Highlights — the brush picker now shows and groups by paintop family

### Brush picker family polish (loop-42)
- Every preset card in the brush picker now carries a paintop FAMILY
  BADGE (paintbrush, spraybrush, eraser, ...), mirroring the editor
  panel chip shipped in v0.27. Presets without a declared family stay
  badge-free.
- New Sort toggle (Name | Family): Family groups presets by paintop
  family alphabetically (name order within a family; presets without a
  declared family sort last) — Krita-parity browsing over the same
  identity data the engine ABI reports.
- Automation hardening: the two heaviest editor tests gained explicit
  2-minute windows, absorbing the suite-parallelism runner contention
  observed since loop-39 (documented with reproduction evidence; a real
  deadlock still fails).
- 3 new widget tests cover the badge rendering, the family ordering and
  the pick callback.

## Also in this build
- Everything from v0.27: krita_brush_get_paintop_id across all bridges
  with CI smoke gates + per-family hardness gating in the panel;
...
visible across the editor UI (this release).
...
def main():
    global APP_RUN_ID
    out_dir = '/home/z/fkr-step1/build/release_v28'
    ...
    payload = json.dumps({
        ...
        'name': 'Feather-Krita v0.28 — brush picker family badges and grouping',
        'body': body,
        'draft': False,
        'prerelease': False,
    }).encode()
